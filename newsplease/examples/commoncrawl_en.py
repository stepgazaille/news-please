...
my_local_download_dir_article = './cc_news/'
        ...
        if detect(text) == 'en':
            with open(__get_pretty_filepath(my_local_download_dir_article, article), 'w', encoding='utf-8') as outfile:
                if my_json_export_style == 0:
                    json.dump({'text': text}, outfile, default=str, separators=(',', ':'), ensure_ascii=False)
                elif my_json_export_style == 1:
                    json.dump({'text': text}, outfile, default=str, indent=4, sort_keys=True, ensure_ascii=False)
